...
public class Processors {
     ...
     */
    public static <A, B, C, D> Processor<A, B, D> compose( Processor<A, C, D> g, Processor<A, B, C> f ) {
        return new ComposedProcessor<A, B, C, D>( g, f );
    }

    /**
     * Wraps a function that transforms a list of input values into a list of output values and is expected to process or persist the data in some way.
    ...
    public static <Input, Output> Processor<Input, Output, FilePersistenceOutputInfo> fileWriter(
            String dirName, FileWriterAdapter<Input, Output> adapter
            ) {
        return new FilePersistence<Input, Output>(dirName, adapter);
    }

    /**
     * Like {@link #fileWriter(String, FileWriterAdapter)}, but additionally writes a control-file that can be used for
    ...
    public static <Input, Output> Processor<Input, Output, ControlFilePersistenceOutputInfo> controlledFileWriter(
            ...
            FileWriterAdapter<Input, Output> adapter
            ) {
    ...
    public static <Input, Output> Processor<Input, Output, BatchProcessorResult<FilePersistenceOutputInfo>> batchFileWriter(
            String dirName, FileWriterAdapter<List<Input>, List<Output>> adapter
            ) {
     ...
     * and the item number within that file (for csv this will probably correspond to the rownum, depending on your implementation of the adapter)
     *
     * This is very similar to {@link #batchFileWriter(String, FileWriterAdapter)}, but for each persisted file there will be a control file as well.
     *
     * @return
     */
    public static <Input, Output> Processor<Input, Output, BatchProcessorResult<ControlFilePersistenceOutputInfo>> controlledBatchFileWriter(
            ...
            FileWriterAdapter<List<Input>, List<Output>> adapter
            ) {
        return new BatchProcessor<Input, Output, ControlFilePersistenceOutputInfo>(controlledFileWriter(dirName, controlFileEnding, adapter));
    }
     ...
     *
     */
    public static <Input> Processor<Input, ControlFile, File> controlledFileMover(String targetDir) {
     ...
     * @return
     */
    public static <Input> Processor<Input, File, File> fileMover(String targetDir) {
